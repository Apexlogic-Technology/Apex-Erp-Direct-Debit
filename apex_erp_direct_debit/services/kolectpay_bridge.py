"""
KolectPayBridge — Bridge Mode implementation for KolectPay Business.

Calls the External API module added to KolectPay Business, authenticated
by a Bearer token stored in DD Settings (bridge_api_token).

Required routes on KolectPay Business (to be implemented in Phase 3):
  POST   /api/external/debtors
  GET    /api/external/debtors/{id}
  POST   /api/external/debts
  GET    /api/external/debts/{id}
  POST   /api/external/debts/{id}/initiate
  POST   /api/external/debts/{id}/verify-otp
  POST   /api/external/debts/{id}/trigger-debit
...
class KolectPayBridge(DirectDebitProviderBase):
	"""Bridge to KolectPay Business REST API."""
	...
	# ─── Mandate lifecycle (delegated to KolectPay) ───────────────────────
	...
	def create_debtor(self, customer_doc) -> dict:
		...
		return self._post("/api/external/debtors", payload)
	...
	def create_debt(self, debt_doc) -> dict:
		...
		payload = {
			...
			"description":     debt_doc.description or debt_doc.narration or "",
			"reference":       debt_doc.name,
			"erpnext_debt_id": debt_doc.name,
		}
		return self._post("/api/external/debts", payload)
	...
	def _post(self, path: str, payload: dict) -> dict:
		...
		try:
			...
			frappe.log_error(title=f"[KolectPay Bridge] POST error: {path}", message=str(e))
			return {"error": str(e)}
	...
	def _get(self, path: str, params: dict = None) -> dict:
		...
		try:
			...
			frappe.log_error(title=f"[KolectPay Bridge] GET error: {path}", message=str(e))
			return {"error": str(e)}
